...
def run(build, client):
    ...
    # Computation
    logging.info(f"Stage 14.1 Building {model} feature matrix")
    pipe.adjust_workers(client, 4)
    ssa.features_from_dataset_zarr(
        ssa.sdf_surfleg.features_from_mask, 
        masks_path, 
        feature_path, 
        **kwargs, 
    )
    ...
    logging.info("Stage 14.3 Compute scores")
    pipe.adjust_workers(client, 1)
    ...
    ssa.modes_from_pca_zarr(
        ssa.sdf_surfleg.mask_from_features, 
        pca_path, 
        modes_path, 
        n_components=8, 
        max_coeff=10,
