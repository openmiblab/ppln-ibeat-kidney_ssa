...
def run(build, logfile, model='spectral'):
    ...
    logging.info(f"Stage 7[{model}] Setup")
    ...
    recon_sections_png = os.path.join(dir_model_output, 'imgs_recon_sections_orders')

    # Output - movies
    recon_mp4 = os.path.join(dir_model_output, 'movie_recon_orders.mp4')

    # Hyperparameters
    n_samples = None # set to None to run all
    n_outliers = 10

    module = MODELS[model]['module']
    kwargs = MODELS[model]['kwargs']

    logging.info(f"Stage 7[{model}] Selecting number of features")
    # pipe.adjust_workers(client, min_ram_per_worker=MODELS[model]['ram'])
    ...
    logging.info(f"Stage 7[{model}] Plotting reconstruction fidelity")
    ...
    logging.info(f"Stage 7[{model}] Displaying reconstruction fidelity")
    ...
    # pipe.adjust_workers(client, min_ram_per_worker=16)
    display.recon_err(recon_masks, recon_png, recon_mp4)
    ssa.plot_mask_sections(
        dataset_zarr_path=recon_masks, 
        dir_png=recon_sections_png, 
    )

    logging.info(f"Stage 7[{model}] Completed")
