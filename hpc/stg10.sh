...
#SBATCH --job-name=stg10
#SBATCH --output=logs/stg10.out
#SBATCH --error=logs/stg10.err
...
rsync -av --no-group --no-perms --delete "$BUILD/kidney_ssa/stage_10_deep_pca/spectral" "$ARCHIVE/kidney_ssa/stage_10_deep_pca"
srun "$ENV/bin/python" "$CODE/stage_10_deep_pca.py" --build="$BUILD" --model="legendre"
rsync -av --no-group --no-perms --delete "$BUILD/kidney_ssa/stage_10_deep_pca/legendre" "$ARCHIVE/kidney_ssa/stage_10_deep_pca"