...
#SBATCH --job-name=stg4-6
#SBATCH --output=logs/stg4-6.out
#SBATCH --error=logs/stg4-6.err
...
srun "$ENV/bin/python" "$CODE/stage_4_extract_features.py" --build="$BUILD"
rsync -av --no-group --no-perms "$BUILD/kidney_ssa/stage_4_extract_features" "$ARCHIVE/kidney_ssa"
srun "$ENV/bin/python" "$CODE/stage_5_export.py" --build="$BUILD"
rsync -av --no-group --no-perms "$BUILD/kidney_ssa/stage_5_export" "$ARCHIVE/kidney_ssa"
srun "$ENV/bin/python" "$CODE/stage_6_distance_matrices.py" --build="$BUILD"
rsync -av --no-group --no-perms "$BUILD/kidney_ssa/stage_6_distance_matrices" "$ARCHIVE/kidney_ssa"